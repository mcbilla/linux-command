...
**sed** 是一种流编辑器，它是文本处理中非常重要的工具，主要用来自动编辑一个或多个文件，简化对文件的反复操作，编写转换程序等。能够完美的配合正则表达式使用，功能不同凡响。
...
sed 基本上就是玩正则模式匹配，所以使用 sed 之前需要对正则表达式有一定的了解。

## 适用的Linux版本

sed可以在大多数Linux发行版中安装和使用，它是GNU项目的一部分。如果你的Linux系统没有预先安装sed。不同的Linux发行版可能使用不同的包管理工具来安装软件包，例如：
...
* CentOS 8、Red Hat Enterprise Linux 8、Fedora
...
command 由地址和编辑命令组成，格式如下：

```
# 两个地址用来标识行的范围
[address[,address]]command

# 单个地址，表示只能读单行，不能应用于某个范围
[line-address]command

# 使用正则表达式来匹配行，对于特殊符号需要用 `\` 进行转义，也可以使用 sed 的 `-r`参数，这样就不需要转义了
/pattern/command
```

sed 命令默认会作用于文本数据的所有行。如果只想将命令作用于特定行或某些行，则必须写明 `address`部分。
