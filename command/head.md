...
head命令是一个用来显示文件开头内容的命令，它可以显示指定文件的前N行或前N个字节。head命令常用于查看日志文件或文本文件的开头部分，以便快速了解文件的基本信息。head命令还可以和其他命令结合使用，比如tail、grep、sed等，实现更多的功能。

## Linux head命令适用的Linux版本
...
* options：如果没有指定 options，默认显示前 10 行。

* files：如果没有指定文件，或者文件为`-`，则从标准输入读取数据。如果指定了多个文件，每个文件的输出之前会加上一个文件名的标题。
